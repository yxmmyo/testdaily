package sevenSort_1207;

public class SevenSort_1207 {
    public static void mergeSort(int[] ary) {
        mergeSortFunction(ary,0,ary.length);
    }

    private static void mergeSortFunction(int[] ary, int left, int right) {
        if (left + 1 >= right) {
            return;
        }
        int mid = (left + right) / 2;
        mergeSortFunction(ary,left,mid);
        mergeSortFunction(ary,mid,right);
